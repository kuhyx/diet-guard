...
/// Answers the way Firebase does for an account that withholds its email.
///
/// `signInWithIdp` simply omits the field -- the session is otherwise
/// identical and perfectly usable.
http_testing.MockClient _signsInWithoutEmail() => http_testing.MockClient(
  (request) async => http.Response(
    jsonEncode({
      'idToken': 'id',
      'refreshToken': 'refresh',
...
void main() {
  ...
  group('openFirebaseWithGoogle', () {
    ...
    test('stores the account even when Firebase reports no email', () async {
      // The regression that cost a night of syncing. `signInWithIdp` omits
      // `email` when the Google account withholds it; the account write used
      // to be gated on that field while the client was returned regardless,
      // so this call reported success, persisted nothing, and the next launch
      // found no account and silently dropped to GitHub-only with a 401.
      //
      // The refresh token -- the thing that actually authenticates -- is
      // already stored by now. An empty email is the honest value and costs
      // nothing: openFirebase() reads the session, not the address.
      final saved = <FirebaseAccount>[];
      final client = await openFirebaseWithGoogle(
        tokenFetcher: () async => 'a-token',
        accountSaver: (account) async => saved.add(account),
        httpClient: _signsInWithoutEmail(),
      );

      expect(client, isNotNull);
      expect(saved, hasLength(1));
      expect(saved.single.email, isEmpty);
      expect(saved.single.password, isEmpty);
      client!.close();
    });
