"""Food-log entry <-> ``crdt_sync.Record`` adapters.

Each entry maps to one ``Record`` with a single opaque ``body`` field holding
everything except ``id``/``deleted``: entries are immutable after creation
(only ``deleted`` ever changes post-write, see
:func:`diet_guard._state.resign_entry`), so the whole body shares one derived
``Hlc``. ``hmac`` travels inside ``body`` but is never trusted on read --
:func:`diet_guard._sync.run_sync` always re-signs after merging.

Backward compatible with devices not yet migrated:
:func:`parse_remote_log` tries the Record-based wire format first and falls
back to the old plain-``DayLog`` format. Push always writes the new format.
"""
